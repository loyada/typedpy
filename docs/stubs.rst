...
Limitations
-----------
1. Any library that manipulates the classes or the modules dynamically is not supported, with the exception of SQLAlchemy.
2. Any class that adds attributes dynamically to the instances - these attributes will obviously not
   be discovered by Typedpy stubs generation
