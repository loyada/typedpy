...
def test_fast_serialization_with_non_typedpy_wrapper_may_fail(
    ...
    with raises(TypeError) as excinfo:
        result = Example.serialize(example)
    assert "Object of type Point2 is not JSON serializable" in str(excinfo.value)


def test_some_empty_fields(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
        a = Integer
        b = String
        _required = []
...
def test_serialize_set(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
        a = Set()
...
def test_string_field_wrapper_compact(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
        st = String
...
def test_string_field_wrapper_not_compact(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
        st = String
        _additionalProperties = False
...
def test_set_field_wrapper_compact2(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
...
def test_serializable_serialize_and_deserialize(no_defensive_copy_on_get):
...
def test_serializable_serialize_and_deserialize2(no_defensive_copy_on_get):
...
def test_serialize_ignore_non_fields_values(no_defensive_copy_on_get):
    from datetime import datetime
...
def test_serialize_map(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
...
def test_serialize_with_mapper_to_different_keys(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
        a = String
        i = Integer
...
def test_serialize_with_deep_mapper_ignores_mappping(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
        a = String
        i = Integer
...
def test_serialize_with_mappers_in_nested_structures(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
        ab_a = String
        ij_i = Integer
...
def test_serialize_with_camel_case_setting(no_defensive_copy_on_get):
    class Bar(Structure, FastSerializable):
        bar_bar = String
...
def test_serialize_with_mapper_with_functions_not_supported(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
        function = Function
        i = Integer
...
def test_enum_serialization_returns_string_name(no_defensive_copy_on_get):
...
def test_serialization_of_classreference_should_work(no_defensive_copy_on_get):
    class Bar(Structure, FastSerializable):
        x = Integer
        y = Integer
...
def test_serialize_array_field_directly(no_defensive_copy_on_get):
    class Values(enum.Enum):
        ABC = enum.auto()
        DEF = enum.auto()
        GHI = enum.auto()
...
def test_convert_camel_case(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
        first_name: String
        last_name: String
...
def test_serialize_field_with_inheritance(no_defensive_copy_on_get):
    class Foo(Structure, FastSerializable):
        s: str
        i: int
        d: DateTime
...
def test_serialize_multified_with_any(no_defensive_copy_on_get):
    class MyPoint2:
        def __init__(self, x, y):
            self.x = x
            self.y = y
    ...
    class Foo(Structure, FastSerializable):
        a: Array[AnyOf[Integer, MyPoint2]]

    create_serializer(Foo)

    foo = Foo(a=[1, MyPoint2(1, 2)])
    with raises(TypeError) as excinfo:
        Foo.serialize(foo)
    assert "Object of type MyPoint2 is not JSON serializable" in str(excinfo.value)


def test_optional_field_defect_234(serialized_example, no_defensive_copy_on_get):
