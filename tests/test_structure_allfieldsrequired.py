...
from typedpy import ImmutableStructure, Map, Structure, mappers, Serializer, AllFieldsRequired
...
def test_allfieldsrequired_of_structure():
    ...
    assert Serializer(bar).serialize() == {'A': [1, 2, 3], 'D': {'x': 1}, 'I': 5, 'S': 'abc', 'X': 'xyz'}

    Bar._serialization_mapper = {"I": "number"}
    assert Serializer(bar).serialize() == {'A': [1, 2, 3], 'D': {'x': 1}, 'S': 'abc', 'X': 'xyz', 'number': 5}
